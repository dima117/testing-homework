...
describe('Тесты общих требований', async function () {
    describe('Интерфейс адаптируется под ширину экрана', async function () {
        ...
        it('Меню ссылок скрывается при ширине меньше 576px', async function ({browser}) {
            await browser.setWindowSize(500, 1000)
            await browser.url('/hw/store');
    ...
    it('Гамбургер-меню закрывается при выборе элемента', async function ({browser}) {
    ...
    it('Ссылки в шапке отображаются', async function ({browser}) {
        ...
        for (let index = 0; index < links.length; index++) {
            ...
            assert.notEqual(elDisplay, false, `Ссылка номер ${index + 1} (${await element.getText()}) не отображается`)
        }
    });
    it('Нажатие по заголовку перенаправляет на главную страницу', async function ({browser}) {
